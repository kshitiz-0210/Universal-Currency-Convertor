...
# Set page config to wide for mobile responsiveness
st.set_page_config(page_title="Universal Currency Converter", page_icon="💱", layout="wide")

# Custom CSS for desktop and responsive design on mobile
st.markdown("""
    <style>
        .main {
...
# Session state for default selections
...
if st.button("💸 Convert"):
    if from_currency == to_currency:
        ...
        try:
            ...
            st.markdown(
                f"<div class='result-box'>💰 {amount:.2f} {from_currency} = {converted:.2f} {to_currency}</div>",
                unsafe_allow_html=True
            )
